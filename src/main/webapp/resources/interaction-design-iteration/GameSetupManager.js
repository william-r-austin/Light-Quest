GameSetupManager = (function() {
		...
		if(RoomInfo.getRoomOwnerKey() === UserInfo.getUserKey()) {
			...
			if(gameUsersCount >= 3) {
				if($("#completeGameSetupDiv").length < 1) {
					...
					$("#gameSetupStatusContainer").append(startGameDivElement);
	...
	function refreshGameSetupUI(gameUserChanges) {
		...
		if(userCount < 1) {
			tempDiv.append("Uh-oh. Your game doesn't have any users! ");
		}
		else if(userCount == 1) {
			tempDiv.append("Looks like it's just you. You'll need a couple more people to join before you can start the game.");
		}
		else if(userCount == 2) {
			tempDiv.append("Almost there! You only need one more person before you can play.");
		}
		else if(userCount == 3) {
			tempDiv.append("Great news: you've got enough people to start the game! The host can choose to start now, or you can " +
			"continue waiting for additional players.");
		}
		else {
			tempDiv.append("You've got " + userCount + " users for your game! That's more than enough to get started, but you can " +
			...
			tempDiv.append(spanHost);
			tempDiv.append(") when you're ready to begin!");
		}
		
		$("#gameStatusSetupMessage").html(tempDiv.html());
