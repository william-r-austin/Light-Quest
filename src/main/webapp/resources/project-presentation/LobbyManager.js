LobbyManager = (function() {
			...
			$("#transitionMessage").show("fade", {}, "slow", function() {
				setTimeout(function() {
					$("#transitionMessage").hide("fade", {}, "slow", function() {
						GameSetupManager.createNewGameRoom(gameType);
					});
				}, 1000);
			});
		});
	...
	function setupListener() {
		if(!initialized) {
			$("#createTextBasedRoomButton").on("click", function(event) {
				createGameRoom("TEXT");
		    });

			$("#createDrawingBasedRoomButton").on("click", function(event) {
				createGameRoom("DRAWING");
			});
