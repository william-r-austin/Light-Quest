NavBarManager = (function() {
	...
	function setTheme(newTheme) {
		...
		// The chat window may need to be re-sized
	...
	function changeNameFormSubmitted(submitFormEvent) {
		...
		if(userName.length > 0) {
			...
			if(userName.length <= Constants.getMaxNameLength()) {
				var originalUserName = UserInfo.getUserName();
				...
				if(originalUserName !== userName) {
					...
					if(roomKey) {
						...
						/*
						var nameChangeChatMessage = {
							"chatType": "SYSTEM",
							"message": 
						};*/
						GameSetupManager.addSystemChatEntry(originalUserName + " has updated their username to: " + userName);
					}
					else {
